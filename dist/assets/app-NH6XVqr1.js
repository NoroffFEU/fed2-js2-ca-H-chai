...
class d {
  ...
  post = {
    ...
    readPost: async (e, t = null) => {
      ...
      const a = await fetch(n.toString(), { headers: v(), method: "GET" });
  ...
  posts = {
    getPosts: async (e = 12, t = 1, n) => {
      ...
      const o = await fetch(a.toString(), { headers: v(), method: "GET" });
...
function Q(p) {
  ...
  e.classList.add("post-container", "group"),
    (e.id = p.id),
    (e.href = `/post/?id=${p.id}`);
  const t = document.createElement("figure");
  t.classList.add("aspect-[4/3]", "overflow-hidden", "rounded-md");
  ...
  E.classList.add("tag-list", "flex", "items-center", "gap-1", "flex-wrap"),
    p.tags
  ...
  f.classList.add("comment", "flex", "justify-between", "items-center");
  const C = document.createElement("i");
  C.classList.add("fa-regular", "fa-comments", "mr-1");
  const x = document.createElement("p");
  (x.textContent = p.comments.length), f.append(C, x), u.append(E, f);
  const L = document.createElement("p");
  L.classList.add("font-bold", "text-2xl", "break-words"),
...
function ee(p) {
  ...
  f.classList.add("font-body", "text-sm"),
    (f.textContent = p.body),
    g.appendChild(f);
  ...
  L.classList.add("comment-list", "pt-4", "lg:px-4", "lg:py-6");
  const S = p.comments,
    B = S.filter((y) => y.replyToId === null);
  for (let y = 0; y < B.length; y++) {
    const b = document.createElement("li");
    b.classList.add("comment-item", "original-comment-item");
    const w = B[y];
    (b.id = w.id), (b.dataset.username = w.author.name);
    ...
    I.classList.add("w-8", "h-8", "rounded-full", "mr-2"),
      (I.src = w.author.avatar.url);
    const P = document.createElement("a");
    P.classList.add("comment-username", "underline"),
      (P.href = `/profile/?name=${w.author.name}`),
      (P.textContent = w.author.name),
      R.append(I, P);
    ...
    U.classList.add("comment-text", "text-sm"), (U.textContent = w.body);
    ...
    H.classList.add("reply-list", "pl-6"), b.append(k, H), L.appendChild(b);
  }
  const T = S.filter((y) => y.replyToId !== null),
    N = p.comments;
  for (let y = 0; y < T.length; y++) {
    const b = document.createElement("li");
    b.classList.add("comment-item", "border-t");
    const w = T[y];
    (b.id = w.id), (b.dataset.username = w.author.name);
    const k = document.createElement("div");
    ...
    if (
      ...
      b.append(k, H),
      N.find((z) => Number(z.id) === w.replyToId))
    ) {
      const z = Array.from(L.querySelectorAll("li.comment-item"));
      Array.from(L.querySelectorAll("ul.reply-list")).forEach((W, ae) => {});
      const A = z.find((W) => Number(W.id) === w.replyToId);
      A
        ? A.querySelector(".reply-list").appendChild(b)
...
Please try again.`);
      ...
      displaySinglePost: async () => {
        try {
          ...
          s.classList.add(
            ...
            (s.textContent = "Edit");
          const l = document.createElement("i");
          l.classList.add("fa-regular", "fa-pen-to-square", "mr-1"),
          ...
          const E = ee(a);
          u.appendChild(E);
          const g = document.querySelector(".back-to-profile-page");
          (g.href = `/profile/?name=${o}`), (g.textContent = `${o}'s page`);
          const f = document.createElement("i");
          f.classList.add("fa-solid", "fa-chevron-left", "mr-1"),
    ...
    profile: {
      displayProfilePage: async (e = 1) => {
        ...
        try {
          ...
          const E = document.querySelector(".button-area"),
          ...
          f.classList.add(
            ...
            E.append(g, f),
            n === d.user
              ? ((g.style.display = "block"), (f.style.display = "none"))
          ...
          window.history.replaceState({}, "", S),
            (this.currentPage = e),
            this.pagination.profilePagination(this.currentPage, L),
            window.scrollTo({ top: 0, behavior: "smooth" }),
  ...
  pagination = {
    feedPagination: async (e, t) => {
      ...
      const a = (r, c) => {
          ...
          return (
            ...
            (u.dataset.page = c),
            (u.className =
              "pagination-button w-9 h-9 border-2 border-green rounded-full hover:bg-light"),
    ...
    singlePostPagination: async () => {
      ...
      if ((m.classList.add("hover:underline"), m))
        if (i > 0) {
          const c = t[i - 1].id;
          m.addEventListener("click", () => {
      ...
      if ((r.classList.add("hover:underline"), r))
        if (i < t.length - 1) {
          const c = t[i + 1]?.id;
          r.addEventListener("click", () => {
          ...
          try {
            let c = await h.posts.getPosts(100, o);
            (t = t.concat(c.data)), (i = t.findIndex((E) => E.id === l));
            const u = t[i + 1]?.id;
            r.addEventListener("click", () => {
              u
